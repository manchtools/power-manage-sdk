 ...
 */
export function parseCreationOptions(optionsJson) {
 ...
 */
export function parseRequestOptions(optionsJson) {
    const options = JSON.parse(optionsJson);
    return {
        publicKey: {
            ...options.publicKey,
...
export async function isConditionalUIAvailable() {
    if (!isWebAuthnAvailable())
        return false;
    try {
        // @ts-expect-error - isConditionalMediationAvailable is not in all TypeScript versions
 ...
 */
export async function authenticateWithPasskey(optionsJson, useConditionalUI = false) {
    if (!isWebAuthnAvailable()) {
        throw new Error('WebAuthn is not supported in this browser');
    }
    const options = parseRequestOptions(optionsJson);
    if (useConditionalUI) {
        // @ts-expect-error - mediation is not in all TypeScript versions
